...
class HomeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //To to long tasks
            ...
            self.keepLoggedIn() {
                ...
                if response == true {
                    print("Hello")
                    print(Constants.classNum)
                    self.getReportCardHTML()
                    self.getTranscriptHTML()
                    self.updateTranscriptGPA(HTML: self.transcriptHTML)
                    print("Hello")
    ...
    func updateTranscriptGPA(HTML: String) {
        do {
            ...
            var weightedGPA = try doc.select("span#plnMain_rpTranscriptGroup_lblGPACum1").text()
            UserDefaults.standard.set(weightedGPA, forKey: "\(UserDefaults.standard.object(forKey: "username") as! String)weightedGPA")
        }
        catch {
